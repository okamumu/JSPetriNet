...
public class DiffOperator extends BinOperator<Boolean> {

	private final Boolean imply(Boolean f, Boolean g) {
		return (!f) || g;
	}

	@SuppressWarnings("unchecked")
	@Override
	protected final Vertex apply(Vertex f, Vertex g, Map<PairVertex, Vertex> hash, Bdd<Boolean> bdd) {
		...
		if (f instanceof Terminal<?> && g instanceof Terminal<?>) {
			...
			result = bdd.createTerm(!imply(fterm.getValue(), gterm.getValue()));
